...
interface Marshal {
  ...
  export default function MarshalDetails() {
    ...
    // --- START OF THE CLEAN RETURN ---
    // إصلاح الوصول إلى typeIcons/typeLabels/typeColors والتأكد من إغلاق جميع العناصر وعدم وجود تكرار أو تداخل
    return (
      <div className="min-h-screen bg-gradient-to-br from-black via-zinc-900 to-black">
        <main className="max-w-5xl mx-auto px-2 sm:px-8 py-10">
          <section className="bg-zinc-900/80 border border-zinc-800 rounded-3xl p-6 md:p-12 shadow-2xl max-w-4xl mx-auto mt-10">
            {/* زر العودة وأزرار التحكم */}
            <div className="flex items-center justify-between mb-6">
              <button
                onClick={() => router.back()}
                className="flex items-center gap-2 px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-gray-200 rounded-lg font-bold text-base shadow border border-zinc-700 transition-all"
              >
              ...
              <div className="flex flex-row-reverse gap-4">
                ...
                <button
                  ...
                  className={`px-5 py-2 rounded-xl font-bold transition-all text-base shadow-md ${marshal && marshal.isActive ? 'bg-yellow-600 hover:bg-yellow-700 text-white' : 'bg-green-600 hover:bg-green-700 text-white'}`}
                  disabled={!marshal}
                >
                  {marshal && marshal.isActive ? '⏸️ إيقاف' : '▶️ تفعيل'}
                </button>
                <button
                      ...
                      {marshal.marshalTypes && marshal.marshalTypes.split(',').filter(t => t).length > 0 ? (
                        marshal.marshalTypes.split(',').filter(t => t).map((type) => {
                          ...
                          const typeColors: Record<string, string> = {
                            'drag-race': 'bg-gradient-to-r from-pink-600 to-red-500 text-white',
                            'motocross': 'bg-gradient-to-r from-yellow-500 to-orange-500 text-white',
                            'karting': 'bg-gradient-to-r from-blue-600 to-cyan-400 text-white',
                            'drift': 'bg-gradient-to-r from-gray-700 to-gray-400 text-white',
                            'circuit': 'bg-gradient-to-r from-green-600 to-lime-400 text-white',
                            'rescue': 'bg-gradient-to-r from-red-700 to-yellow-400 text-white'
                          };
                          return (
                            <span key={type} className={`inline-flex items-center gap-1 px-4 py-1 rounded-full text-base font-bold border border-zinc-700 shadow-sm ${(typeColors[type] ?? 'bg-zinc-800 text-white')}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-12">
              <div>
                ...
                {marshal && marshal.licenseFrontImage ? (
              ...
              <div>
                ...
                {marshal && marshal.licenseBackImage ? (
            ...
            >
              ...
              <p className="text-gray-300 mb-6">
                {language === "ar" && marshal ?
                  ...
                  marshal ? `Are you sure you want to delete marshal "${marshal.name}"? This action cannot be undone.` : ''
                }
              </p>
