...
export async function POST(
  ...
  try {
    ...
    if (attendance.userId !== session.user.id) {
      return NextResponse.json({ error: "لا يمكنك قبول طلب غير خاص بك" }, { status: 403 })
    }

    // التحقق من أن الطلب في حالة معلق
    if (attendance.status !== "pending") {
      return NextResponse.json({ error: "يمكن قبول الطلبات المعلقة فقط" }, { status: 400 })
    }
    ...
    return NextResponse.json({
      message: "تم قبول طلب الحضور بنجاح",
